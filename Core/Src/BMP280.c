...
BMP280_calib_data dig;
 ...
 void I2C_writeByte(I2C_HandleTypeDef hi2c1,uint8_t device_address,uint8_t register_address,uint8_t * wdata,uint8_t size)
 {
 	i2c_status = HAL_I2C_Mem_Write(&hi2c1,device_address << 1,register_address,1,wdata,size,10*size);
 }
 void I2C_readByte(I2C_HandleTypeDef hi2c1,uint8_t device_address,uint8_t register_address,uint8_t * rdata,uint8_t size)
 {
 	i2c_status = HAL_I2C_Mem_Read(&hi2c1,device_address << 1,register_address,1,rdata,size,size*10);
 }
 
uint8_t BMP280_getID()
{
	uint8_t address = 0;
	I2C_readByte(hi2c2,BMP280_ADDRESS,BMP280_ID,&address,1);
	return address;
